...
namespace WarTornLands.Infrastructure
{
    class Interface : DrawableGameComponent
    {
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            for (int i = 0; i < (Game as Game1).Player.Inventory.Potions; ++i)
            {
                (Game as Game1).SpriteBatch.Draw(_potionTexture, new Rectangle(10 + 42 * i, 10, 32, 32), Color.White);
            }

            // Display health
            String text = (Game as Game1).Player.Health.ToString();
            (Game as Game1).SpriteBatch.DrawString(_font, text, new Vector2(Game.Window.ClientBounds.Width -
                _font.MeasureString(text).X - 47, 10), Color.White);
            (Game as Game1).SpriteBatch.Draw(_heartTexture, new Rectangle(Game.Window.ClientBounds.Width - 42, 10, 32, 32), Color.White);

            // Display game over
            if ((Game as Game1).Player.Health == 0)
            {
                // Viel schwarzer Text für Rand
                (Game as Game1).SpriteBatch.DrawString(_font, "GAME OVER", new Vector2(Game.Window.ClientBounds.Width / 2 -
