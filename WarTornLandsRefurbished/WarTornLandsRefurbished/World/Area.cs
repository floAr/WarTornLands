    ...
    public class Area
    {
        ...
        public List<Entity> GetEntitiesAt(Vector2 position)
        {
            ...
            foreach (Layer layer in _layers)
            {
                ...
                {
                    result = (List<Entity>)result.Concat((layer as EntityLayer).GetEntitiesAt(position));
                }
            }
        ...
        public List<Entity> GetEntitiesAt(Vector2 position, float radius)
        {
            ...
            foreach (Layer layer in _layers)
            {
                if (layer is EntityLayer)
                {
                    result = (List<Entity>)result.Concat((layer as EntityLayer).GetEntitiesAt(position, radius));
                }
            }
